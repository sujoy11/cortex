...
export function getToken(): string | null {
  ...
  // No-login builds (VITE_DISABLE_AUTH) fall back to a build-time bootstrap
  // admin token so the dashboard can call the API without a login screen.
  if (import.meta.env.VITE_DISABLE_AUTH === 'true' && import.meta.env.VITE_BOOTSTRAP_TOKEN) {
    return import.meta.env.VITE_BOOTSTRAP_TOKEN as string;
  }
  return null;
}
