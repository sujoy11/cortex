...
[![CI](https://github.com/tashfeenahmed/CORTEX/actions/workflows/ci.yml/badge.svg)](https://github.com/tashfeenahmed/CORTEX/actions/workflows/ci.yml)
[![GitHub stars](https://img.shields.io/github/stars/tashfeenahmed/CORTEX?style=flat&logo=github&color=yellow)](https://github.com/tashfeenahmed/CORTEX/stargazers)
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](./LICENSE)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](#contributing)
[![Docker image](https://img.shields.io/badge/ghcr.io-cortex-2496ED?logo=docker&logoColor=white)](https://github.com/tashfeenahmed/CORTEX/pkgs/container/CORTEX)
[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/tashfeenahmed/CORTEX)

**[cortex.co](https://cortex.dev/?utm_source=github&utm_medium=readme&utm_campaign=repository&utm_content=readme_top)** · browse the full catalog: 251 model families, 358 free endpoints

**English** · [简体中文](docs/i18n/zh-CN/README.md)

<p align="center">
  <a href="https://play.google.com/store/apps/details?id=co.cortex.app"><img src="repo-assets/badges/play-store.svg" height="60" alt="Get it on Google Play"></a>
  <a href="https://github.com/tashfeenahmed/CORTEX/releases/latest"><img src="repo-assets/badges/macos.svg" height="60" alt="Download for macOS"></a>
  <a href="https://github.com/tashfeenahmed/CORTEX/releases/latest"><img src="repo-assets/badges/windows.svg" height="60" alt="Download for Windows"></a>
  <a href="docs/install.md#docker-compose"><img src="repo-assets/badges/docker.svg" height="60" alt="Self-host with Docker"></a>
</p>

![cortex dashboard — Models page with the monthly token budget](repo-assets/github-hero.png)
...
**[Go live at cortex.co](https://cortex.dev/?utm_source=github&utm_medium=readme&utm_campaign=premium&utm_content=readme_top#pricing)** ($19/yr, cancel anytime).
...
The problem is that stacking them by hand is painful: twenty-nine different SDKs, twenty-nine different rate limits, twenty-nine places a request can fail. cortex collapses that into one OpenAI-compatible endpoint. Point any OpenAI client library at your local server, and it routes transparently across whichever providers you've added keys for.

And the free-tier landscape shifts weekly: providers launch models, retire them, and change quotas without notice. cortex tracks all of that for you. The router pulls a signed model catalog from [cortex.co](https://cortex.dev) on its own, so your install keeps up without a `git pull`. See [Premium (live catalog)](#premium-live-catalog) for how fast it keeps up.
...
The full, always-current list lives at **[cortex.co/models](https://cortex.dev/models.html)** with per-model rate limits, context windows, and free-token budgets.
...
Most of these configure themselves with one command — `npx cortex setup-claude`, `setup-codex`, `setup-aider`, and ten more generators that fetch your live catalog, back up existing config, and never clobber what's already there. Claude Code and Codex also get zero-persistence launchers (`cortex launch`, `cortex launch-codex`) that inject credentials into the child process only. Zed and JetBrains AI connect through the opt-in [Ollama emulation](docs/clients.md#ollama-clients); Gemini CLI speaks its native wire on `/v1beta`.
...
- **Self-updating model catalog** — the router syncs a signed catalog from cortex.co twice a day: new models, quota changes, and provider quirk fixes land automatically. [Premium →](#premium-live-catalog)
...
- **Encrypted keys, one token out** — provider keys are AES-256-GCM encrypted in SQLite and decrypted in-memory per request; your apps only ever see a single unified `cortex-…` bearer token.
...
**One-liner** (Docker required — sets up `~/CORTEX`, generates an encryption key, pulls the image, and starts the container):

```bash
curl -fsSL https://cortex.dev/install.sh | bash
```

Prefer to read before you pipe to bash? [The script is here](https://cortex.dev/install.sh). Re-running it is safe: your `.env` (and encryption key) is preserved and the container updates to `:latest`.
...
On Windows, the easiest path is the desktop **[`.exe` installer from Releases](https://github.com/tashfeenahmed/CORTEX/releases/latest)** (below). On Android, see the experimental [Termux guide](docs/install/android-termux.md).
...
```bash
npx cortex setup-claude --url http://localhost:3001 --api-key <unified-key>
```

Every generator supports `--dry-run`, creates a timestamped backup before changing an existing file, and merges into the user's configuration. Launchers keep credentials out of config files entirely: `npx cortex launch` for Claude Code and `npx cortex launch-codex` for Codex.
...
cortex is local-first and single-user by design. Your provider keys stay in your SQLite database, encrypted at rest, and requests go from your machine to the upstream providers you enabled.
...
from [cortex.co](https://cortex.dev) twice a day and applies new models,
...
**[cortex.co/models](https://cortex.dev/models.html)**.
...
**[Go live at cortex.co →](https://cortex.dev/?utm_source=github&utm_medium=readme&utm_campaign=premium&utm_content=readme_bottom#pricing)**
...
- Activate in the dashboard under **Premium**; cancel or manage billing
  self-serve at [cortex.co/manage](https://cortex.dev/manage).
...
client = OpenAI(
    ...
    api_key="cortex-your-unified-key",
)
...
**This project is for personal experimentation and learning, not production.** Free tiers exist so developers can prototype against them; they aren't a stable, supported inference substrate and shouldn't be treated as one. If you build something real on top of cortex, swap in a paid API before you ship. Your relationship with each upstream provider is governed by the terms you accepted when you created your account — those terms still apply when the traffic is proxied through this project, and you're responsible for complying with them.
