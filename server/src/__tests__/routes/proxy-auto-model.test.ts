...
describe('Virtual "auto" model', () => {
  ...
  beforeEach(async () => {
    ...
    // (owned_by = cortex, id = canonical slug) that aggregates availability
  ...
  it('returns the whole catalog by default, each tagged with availability (#242)', async () => {
    ...
    // Every logical model is owned_by cortex (unified). Only a groq key is
    ...
    expect(models.every((m: any) => m.owned_by === 'cortex')).toBe(true);
