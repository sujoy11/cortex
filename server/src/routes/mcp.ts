...
const TOOLS: Record<string, McpTool> = {
  list_models: {
    description: 'List the models this cortex router can serve, with context windows, tool support, and the parameters each model honors (supported_parameters). Defaults to only models that are usable right now.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  switch (msg.method) {
    case 'initialize': {
      return rpcResult(id, {
        ...
        instructions: 'cortex gateway introspection: list usable free models (with per-model supported_parameters), check provider/key health, read usage/cache/compression stats, and switch the routing strategy. Inference goes through the OpenAI-compatible /v1 endpoints, not MCP.',
      });
    }
    case 'ping':
      return rpcResult(id, {});
