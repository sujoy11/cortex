...
keysRouter.get('/export', (req: Request, res: Response) => {
  ...
  if (format === 'env') {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="cortex-keys.env"');
    res.send(content);
    return;
  }
  ...
  if (format === 'csv') {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="cortex-keys.csv"');
    res.send(content);
    return;
  }
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="cortex-keys.json"');
  res.json(jsonExport);
});
