...
const REPOSITORY = 'tashfeenahmed/CORTEX';
...
export function createUpdateRouter(options: UpdateRouterOptions = {}): Router {
  ...
  const updateCheckDisabled = env.CORTEX_UPDATE_CHECK?.trim().toLowerCase() === 'off';
  const githubToken = env.CORTEX_UPDATE_GITHUB_TOKEN?.trim();
  ...
  async function resolveIdentity(): Promise<Identity> {
    ...
    const environmentSha = validSha(env.CORTEX_COMMIT_SHA);
    const installMethod = env.CORTEX_INSTALL_METHOD;
