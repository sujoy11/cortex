...
Lean Electron menu-bar app for [cortex](https://github.com/tashfeenahmed/CORTEX):
...
npm run desktop:dist:win      # Windows → desktop/dist-electron/cortex-…-x64 installer
```
...
- DB + config live in `~/Library/Application Support/CORTEX/` (macOS) /
  `%APPDATA%/CORTEX/` (Windows).
