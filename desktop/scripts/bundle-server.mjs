...
// and flattens the @cortex/shared workspace symlink — only better-sqlite3
...
await build({
  ...
  define: {
    'process.env.CORTEX_COMMIT_SHA': JSON.stringify(hasBuildIdentity ? githubSha : ''),
    'process.env.CORTEX_INSTALL_METHOD': JSON.stringify(hasBuildIdentity ? 'desktop' : ''),
  },
