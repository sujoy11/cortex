...
app.setName('cortex');
app.setPath('userData', path.join(app.getPath('appData'), 'cortex'));
...
if (!app.requestSingleInstanceLock()) {
  ...
  async function toggleLanAccess(): Promise<void> {
    ...
    if (enabling) {
      const { response } = await dialog.showMessageBox({
        ...
        message: 'Expose cortex to your local network?',
        detail:
  ...
  app.whenReady().then(async () => {
    ...
    try {
      ...
      console.log(`[desktop] cortex running on http://${host}:${port}${cfg.lanAccess ? ' (LAN access enabled)' : ''}`);
