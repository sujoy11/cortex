...
function tempHome(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cortex-doctor-'));
  temporary.push(dir);
  return dir;
}
afterEach(() => {
...
describe('codex precedence', () => {
  ...
  it('reads the dotted-key spelling too', () => {
    ...
    writeCodexConfig(home, [
      'model_provider = "cortex"',
      'model_providers.cortex.base_url = "http://localhost:3000/v1"',
    ].join('\n'));
  ...
  it('records a selected provider that names no base_url instead of skipping it', () => {
    const home = tempHome();
    writeCodexConfig(home, 'model_provider = "cortex"\n');
