...
export function parseArgs(argv: string[]): { command?: string; options: CliOptions } {
  const options: CliOptions = {
    url: process.env.CORTEX_URL || 'http://localhost:3000',
...
async function promptForKey(): Promise<string> {
  if (!process.stdin.isTTY) {
    throw new Error(
      'No API key supplied. Pass --api-key or set CORTEX_API_KEY.',
    );
  }
  let muted = false;
  ...
  try {
    const answer = rl.question('cortex unified API key: ');
    muted = true;
...
async function catalog(url: string, apiKey: string, availableOnly = true): Promise<CatalogModel[]> {
  ...
  try {
    ...
    throw new Error(
      `Could not reach the cortex gateway at ${rootUrl(url)} (${reason}). `
      + 'Check that the server is running, or point the CLI at it with --url or CORTEX_URL.',
    );
  }
  if (!response.ok) {
...
export function resolvePinnedModel(
  ...
  if (rosters.degradedReason) {
    warn(
      `cortex: could not fetch the unfiltered model catalog (${rosters.degradedReason}). `
  ...
  if (resolved.unavailable) {
    warn(
      `cortex: '${resolved.id}' is registered but not currently available `
...
function help(): string {
  return [
    'cortex coding-agent setup',
    '',
    'Usage:',
    '  cortex <command> [--url URL] [--api-key KEY] [--profile NAME] [--model ID] [--dry-run]',
    '',
    'Commands:',
  ...
  if (options.profile !== 'default') {
    ...
    if (!fs.existsSync(directory)) {
      throw new Error(
        ...
        + `Create it with: cortex setup-claude --profile ${options.profile}`,
      );
    }
...
export function codexArgs(url: string, selected: string): string[] {
  return [
    '-c', 'model_provider="cortex"',
    '-c', `model=${JSON.stringify(selected)}`,
    '-c', 'model_providers.cortex.name="cortex"',
    '-c', `model_providers.cortex.base_url=${JSON.stringify(`${rootUrl(url)}/v1`)}`,
    '-c', 'model_providers.cortex.wire_api="responses"',
    '-c', 'model_providers.cortex.env_key="CORTEX_API_KEY"',
    '-c', 'model_providers.cortex.requires_openai_auth=false',
  ];
}
...
async function launchCodex(options: CliOptions): Promise<number> {
  ...
  const env = { ...process.env, CORTEX_API_KEY: apiKey };
