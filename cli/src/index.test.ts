...
describe('CLI arguments and launchers', () => {
  ...
  it('never uses the profile name as a model id', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'cortex-cli-'));
    temporary.push(home);
  ...
  it('rejects a profile whose configuration directory does not exist', () => {
    ...
    expect(() => claudeLaunchEnv(
      ...
      {},
      home,
    )).toThrow("cortex setup-claude --profile missing");
  });
  ...
  it('passes a complete Responses provider to Codex', () => {
    ...
    expect(args).toContain('model_provider="cortex"');
    expect(args).toContain('model="coder"');
    expect(args).toContain('model_providers.cortex.name="cortex"');
    expect(args).toContain(
      'model_providers.cortex.base_url="http://localhost:3001/v1"',
    );
    expect(args).toContain('model_providers.cortex.wire_api="responses"');
  });
});
