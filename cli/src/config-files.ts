...
function markerPattern(): RegExp {
  return new RegExp(
    `^[ \\t]*(?:#|//) cortex:start[\\s\\S]*?^[ \\t]*(?:#|//) cortex:end\\s*`,
    'm',
  );
}

function mergeMarked(existing: string, generated: string): string {
  const start = generated.match(/^[ \t]*(?:#|\/\/) cortex:start$/m)?.[0];
  const end = generated.match(/^[ \t]*(?:#|\/\/) cortex:end$/m)?.[0];
...
function continueModelLines(generated: string): string[] {
  ...
  const endIndex = lines.findIndex(
    (line, index) => index > modelsIndex && line === '# cortex:end',
  );
...
function mergeContinueYaml(existing: string, generated: string): string {
  ...
  const entryStart = lines.findIndex(
    (line, index) => index > modelsIndex
      && index < modelsEnd
      && /^\s{2}-\s+name:\s+["']?cortex["']?\s*$/.test(line),
  );
  if (entryStart >= 0) {
...
function mergeYaml(existing: string, generated: string): string {
  if (/^name: cortex$/m.test(generated) && /^models:$/m.test(generated)) {
  ...
  if (/^# cortex:start$/m.test(existing)) return mergeMarked(existing, generated);
...
function isTomlMarkerLine(line: string): boolean {
  return /^[ \t]*# cortex:(?:start|end)$/.test(line);
}
...
function commitPlannedWrite(plan: PlannedWrite): ApplyResult {
  ...
  const temporary = `${file.path}.cortex-${process.pid}.tmp`;
