...
Docker Compose is the recommended way to run cortex for personal use. The container serves the Express API and the built React dashboard from one process on port 3001, with SQLite persisted in a named volume.
...
Open http://localhost:3001, add provider keys on the **Keys** page, then use the generated `cortex-...` key with any OpenAI-compatible client.

## Example API Call

```bash
curl http://localhost:3001/v1/chat/completions \
  -H "Authorization: Bearer cortex-your-unified-key" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "auto",
    "messages": [{"role": "user", "content": "Say hello from cortex."}]
  }'
```
...
| `FREEAPI_DB_PATH` | No | `/app/server/data/CORTEX.db` | SQLite file path. Set this when your host only persists one mounted directory. |
...
The `cortex-data` volume stores SQLite data at `/app/server/data`. Keep the same volume and `ENCRYPTION_KEY` when upgrading, otherwise existing encrypted provider keys cannot be decrypted.
...
```bash
docker pull ghcr.io/tashfeenahmed/CORTEX:latest
```
