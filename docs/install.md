...
Everything about getting cortex running: the one-liner, Docker Compose, local development, declarative config, production builds, the desktop app, and where your data lives.
...
Docker required — sets up `~/CORTEX`, generates an encryption key, pulls the image, and starts the container:

```bash
curl -fsSL https://cortex.dev/install.sh | bash
```

Prefer to read before you pipe to bash? [The script is here](https://cortex.dev/install.sh). Re-running it is safe: your `.env` (and encryption key) is preserved and the container updates to `:latest`. Override the defaults with `CORTEX_DIR`, `PORT`, or `HOST_BIND` env vars.

On Windows, the easiest path is the desktop **[`.exe` installer from Releases](https://github.com/tashfeenahmed/CORTEX/releases/latest)** ([below](#desktop-app)); the Docker steps work in WSL or any bash shell.
...
Your install keeps itself updated from the signed catalog feed. The current full catalog is listed at [cortex.co/models](https://cortex.dev/models.html).
...
*On macOS / Linux (Bash):*
```bash
git clone https://github.com/tashfeenahmed/CORTEX.git
cd cortex
...
>
> ```bash
> docker compose exec cortex node -e "fetch('https://generativelanguage.googleapis.com/').then(r=>console.log('ok',r.status)).catch(e=>console.log('fail',e.cause?.code||e.message))"
> ```
...
For repeatable Docker/server installs, cortex can apply a JSON config on
every boot. Set `FREEAPI_CONFIG_PATH=/path/to/cortex.devnfig.json` or put the
...
cortex publishes a single production image that contains the Express server and the built React dashboard:

```bash
docker pull ghcr.io/tashfeenahmed/CORTEX:latest   # or pin a release, e.g. :v1.2.3
```
...
SQLite data is stored in the `cortex-data` volume at `/app/server/data`.
...
directory, set `FREEAPI_DB_PATH=/that/path/CORTEX.db`.

On hosts with ephemeral disks, configure an encrypted backup target:

```env
FREEAPI_DB_BACKUP_PATH=/app/server/data/CORTEX.db.backup
# or:
FREEAPI_DB_BACKUP_URL=https://example.com/CORTEX.db.backup
...
When the database file is missing at startup, cortex restores the backup
...
**[Download from Releases](https://github.com/tashfeenahmed/CORTEX/releases/latest)** — the macOS `.dmg` and the Windows `.exe` installer are built and attached to every release by the [`desktop-release`](../.github/workflows/desktop-release.yml) workflow. Or build it from this repo in a few minutes:
...
npm run desktop:dist          # macOS  → desktop/dist-electron/cortex-…-arm64.dmg
npm run desktop:dist:win      # Windows → "desktop/dist-electron/CORTEX Setup ….exe"
```
...
`cortex-…` token your OpenAI/Anthropic client points at. Get it from:
...
| OS | Location |
|----|----------|
| Windows | `%APPDATA%\cortex\` (e.g. `C:\Users\<you>\AppData\Roaming\cortex\`) |
