...
> Experimental and community-supported. cortex runs locally on the Android device.

cortex can run in [Termux](https://termux.dev/) without an Android NDK toolchain. On Android, the server uses Node's built-in SQLite driver instead of the native `better-sqlite3` package.
